...
@section('content')
    <div class="container">
        <div class="card mb-4">
            <div class="card-body">
                <div class="card-title mb-3">Edit {{$product->name}} Product</div>
                <form enctype="multipart/form-data" method="POST" action="{{route('products.update',$product->id)}}">
                    ...
                    <div class="row">
                        <div class="col-md-6 form-group mb-3">
                            ...
                            <input class="form-control" id="name"  value="{{$product->name}}"  name="name" type="text"
                                   placeholder="Enter Name" required/>
                        </div>
                        <div class="col-md-6 form-group mb-3">
                            <label for="firstName1">Enter Detail</label>
                            <textarea class="form-control" name="details" value="{{$product->details}}"   required></textarea>
                        </div>


                        <div class="col-md-6 form-group mb-3">
                            <label for="firstName1">Enter Model</label>
                            <input class="form-control" id="date" name="model" value="{{$product->model}}"  type="text" placeholder="Enter Product Model" required/>
                        </div>

                        <div class="col-md-6 form-group mb-3">
                            <label for="firstName1">Enter Serial Number</label>
                            <input class="form-control" id="name" name="serial_number" value="{{$product->serial_number}}"   type="text"
                                   placeholder="Enter Sale Price" required/>
                        </div>
                        <div class="col-md-6 form-group mb-3">
                            <label for="firstName1">Enter Sale Price</label>
                            <input class="form-control" id="name" name="sale_price" value="{{$product->sale_price}}"  step="0" min="0.00" type="number"
                                   placeholder="Enter Sale Price" required/>
                        </div>
                        ...
                        <div class="col-md-6 form-group mb-3">
                            ...
                            <input class="form-control" id="name" name="supplier_price" step="0" min="0.00" type="number"
                                   placeholder="Enter Supplier Price"  value="{{$product->supplier_price}}" required/>
                        </div>
                        ...
                        <div class="col-md-6 form-group mb-3">
                            ...
                            <select name="tax_id" id="" class="form-control">
                                @foreach($taxes as $item)
                                    <option value="{{$item->id}}" {{$item->id == $product->tax_id ? 'selected' : ''}}>{{$item->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 form-group mb-3">
